...
axiom (forall seed1,seed2 : int :: seed1 != seed2 ==> $2_tx_context_native_sender(seed1) != $2_tx_context_native_sender(seed2));
...
// fun auction::variso_ctx [verification] at ./sources/auction.move:180:5+126
procedure {:timeLimit 45} $0_auction_variso_ctx$verify(_$t0: $Mutation ($2_tx_context_TxContext), _$t1: $Mutation ($2_tx_context_TxContext)) returns ($ret0: $Mutation ($2_tx_context_TxContext), $ret1: $Mutation ($2_tx_context_TxContext))
{
    // declare local variables
    ...
    // $t2 := read_ref($t0) at ./sources/auction.move:181:19+4
    assume {:print "$at(255,8690,8694)"} true;
    $t2 := $Dereference($t0);

    // $t3 := tx_context::sender($t2) at ./sources/auction.move:181:19+13
    $t3 := $2_tx_context_sender$pure($t2, 0);
    assume $IsValid'address'($t3);

    // $t4 := read_ref($t1) at ./sources/auction.move:181:36+4
    assume {:print "$at(255,8707,8711)"} true;
    $t4 := $Dereference($t1);

    // $t5 := tx_context::sender($t4) at ./sources/auction.move:181:36+13
    $t5 := $2_tx_context_sender$pure($t4, 1);
    assume $IsValid'address'($t5);

    // $t6 := !=($t3, $t5) at ./sources/auction.move:181:33+2
    assume {:print "$at(255,8704,8706)"} true;
    $t6 := !$IsEqual'address'($t3, $t5);

    // $t7 := !($t6) at ./sources/auction.move:181:17+1
    call $t7 := $Not($t6);

    // prover::ensures($t7)no_abort check at ./sources/auction.move:181:9+42
    assert {:msg "assert_failed(255,8680,8722): prover::ensures does not hold"} $t7;
    assert {:msg "assert_failed(255,8680,8722): code should not abort"} !$abort_flag;

    // trace_local[ctx1]($t0) at ./sources/auction.move:181:51+1
    assume {:print "$at(255,8722,8723)"} true;
    $temp_0'$2_tx_context_TxContext' := $Dereference($t0);
    assume {:print "$track_local(105,17,0,$2_tx_context_TxContext):", $temp_0'$2_tx_context_TxContext'} $temp_0'$2_tx_context_TxContext' == $temp_0'$2_tx_context_TxContext';

    // trace_local[ctx2]($t1) at ./sources/auction.move:181:51+1
    $temp_0'$2_tx_context_TxContext' := $Dereference($t1);
    assume {:print "$track_local(105,17,1,$2_tx_context_TxContext):", $temp_0'$2_tx_context_TxContext'} $temp_0'$2_tx_context_TxContext' == $temp_0'$2_tx_context_TxContext';

    // return () at ./sources/auction.move:181:51+1
    $ret0 := $t0;
    $ret1 := $t1;
    return;
