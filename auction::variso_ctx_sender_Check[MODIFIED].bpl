...
axiom (forall seedA, seedB : int :: seedA != seedB ==> $2_tx_context_native_sender(seedA) != $2_tx_context_native_sender(seedB));
...
// fun tx_context::sender [baseline] at /home/fran_cueto/.move/https___github_com_asymptotic-code_sui_git_next/crates/sui-framework/packages/sui-framework/sources/tx_context.move:35:1+69
procedure {:inline 1} $2_tx_context_sender(_$t0: $2_tx_context_TxContext, seed : int) returns ($ret0: int)
{
    ...
    $t1 := $2_tx_context_native_sender(seed);
...
// fun auction::variso_ctx_sender [verification] at ./sources/auction.move:185:5+307
procedure {:timeLimit 45} $0_auction_variso_ctx_sender$verify(_$t0: $Mutation ($2_tx_context_TxContext), _$t1: $Mutation ($2_tx_context_TxContext)) returns ($ret0: $Mutation ($2_tx_context_TxContext), $ret1: $Mutation ($2_tx_context_TxContext))
{
    ...
    // bytecode translation starts here
    // assume WellFormed($t0) at ./sources/auction.move:185:5+1
    assume {:print "$at(255,8748,8749)"} true;
    assume $IsValid'$2_tx_context_TxContext'($Dereference($t0));

    // assume WellFormed($t1) at ./sources/auction.move:185:5+1
    assume $IsValid'$2_tx_context_TxContext'($Dereference($t1));

    // trace_local[ctx1]($t0) at ./sources/auction.move:185:5+1
    ...
    // trace_local[ctx2]($t1) at ./sources/auction.move:185:5+1
    ...
    // $t2 := read_ref($t0) at ./sources/auction.move:186:19+4
    assume {:print "$at(255,8842,8846)"} true;
    $t2 := $Dereference($t0);

    // $t3 := read_ref($t1) at ./sources/auction.move:186:27+4
    $t3 := $Dereference($t1);

    // $t4 := !=($t2, $t3) at ./sources/auction.move:186:24+2
    $t4 := !$IsEqual'$2_tx_context_TxContext'($t2, $t3);

    // prover::requires($t4)no_abort check at ./sources/auction.move:186:9+24
    call $0_prover_requires($t4);
    assert {:msg "assert_failed(255,8832,8856): code should not abort"} !$abort_flag;

    // $t5 := read_ref($t0) at ./sources/auction.move:188:17+4
    assume {:print "$at(255,8929,8933)"} true;
    $t5 := $Dereference($t0);

    // $t6 := tx_context::sender($t5) at ./sources/auction.move:188:17+13
    $t6 := $2_tx_context_sender$pure($t5, 0);
    assume $IsValid'address'($t6);

    // $t7 := read_ref($t1) at ./sources/auction.move:188:34+4
    assume {:print "$at(255,8946,8950)"} true;
    $t7 := $Dereference($t1);

    // $t8 := tx_context::sender($t7) at ./sources/auction.move:188:34+13
    $t8 := $2_tx_context_sender$pure($t7, 1);
    assume $IsValid'address'($t8);

    // $t9 := !=($t6, $t8) at ./sources/auction.move:188:31+2
    assume {:print "$at(255,8943,8945)"} true;
    $t9 := !$IsEqual'address'($t6, $t8);

    // prover::ensures($t9)no_abort check at ./sources/auction.move:188:9+39
    assert {:msg "assert_failed(255,8921,8960): prover::ensures does not hold"} $t9;
    assert {:msg "assert_failed(255,8921,8960): code should not abort"} !$abort_flag;

    // trace_local[ctx1]($t0) at ./sources/auction.move:188:48+1
    assume {:print "$at(255,8960,8961)"} true;
    $temp_0'$2_tx_context_TxContext' := $Dereference($t0);
    assume {:print "$track_local(105,17,0,$2_tx_context_TxContext):", $temp_0'$2_tx_context_TxContext'} $temp_0'$2_tx_context_TxContext' == $temp_0'$2_tx_context_TxContext';

    // trace_local[ctx2]($t1) at ./sources/auction.move:188:48+1
    $temp_0'$2_tx_context_TxContext' := $Dereference($t1);
    assume {:print "$track_local(105,17,1,$2_tx_context_TxContext):", $temp_0'$2_tx_context_TxContext'} $temp_0'$2_tx_context_TxContext' == $temp_0'$2_tx_context_TxContext';

    // return () at ./sources/auction.move:188:48+1
    $ret0 := $t0;
    $ret1 := $t1;
    return;
