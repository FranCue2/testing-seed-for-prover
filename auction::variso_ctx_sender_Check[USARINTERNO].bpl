...
// fun ghost::havoc_global<#0, #1> [baseline] at /home/fran_cueto/.move/https___github_com_asymptotic-code_sui-prover_git_main/packages/prover/sources/ghost.move:29:1+32
procedure {:inline 1} $0_ghost_havoc_global'#0_#1'() returns ()
{
    havoc $global_var__'#0_#1';
}

// struct tx_context::TxContext at /home/fran_cueto/.move/https___github_com_asymptotic-code_sui_git_next/crates/sui-framework/packages/sui-framework/sources/tx_context.move:18:1+474
...
// fun tx_context::sender [baseline] at /home/fran_cueto/.move/https___github_com_asymptotic-code_sui_git_next/crates/sui-framework/packages/sui-framework/sources/tx_context.move:35:1+69
...
procedure {:inline 1} $2_tx_context_sender(_$t0: $2_tx_context_TxContext) returns ($ret0: int)
{
    ...
    // $t1 := tx_context::native_sender() at /home/fran_cueto/.move/https___github_com_asymptotic-code_sui_git_next/crates/sui-framework/packages/sui-framework/sources/tx_context.move:36:5+15
    assume {:print "$at(151,1205,1220)"} true;
    $t1 := _$t0->$sender;
    assume $IsValid'address'($t1);

    // return $t1 at /home/fran_cueto/.move/https___github_com_asymptotic-code_sui_git_next/crates/sui-framework/packages/sui-framework/sources/tx_context.move:36:5+15
    assume {:print "$at(151,1205,1220)"} true;
    $ret0 := $t1;
    return;
...
// struct object::ID at /home/fran_cueto/.move/https___github_com_asymptotic-code_sui_git_next/crates/sui-framework/packages/sui-framework/sources/object.move:67:1+390
datatype $2_object_ID {
    $2_object_ID($content: int)
}
function {:inline} $IsValid'$2_object_ID'(s: $2_object_ID): bool {
    true
}
...
// struct object::UID at /home/fran_cueto/.move/https___github_com_asymptotic-code_sui_git_next/crates/sui-framework/packages/sui-framework/sources/object.move:81:1+43
datatype $2_object_UID {
    $2_object_UID($content: int)
}
function {:inline} $IsValid'$2_object_UID'(s: $2_object_UID): bool {
    true
}
...
// fun auction::variso_ctx_sender [verification] at ./sources/auction.move:180:5+180
procedure {:timeLimit 45} $0_auction_variso_ctx_sender$verify(_$t0: $Mutation ($2_tx_context_TxContext), _$t1: $Mutation ($2_tx_context_TxContext)) returns ($ret0: $Mutation ($2_tx_context_TxContext), $ret1: $Mutation ($2_tx_context_TxContext))
{
    // declare local variables
    ...
    // bytecode translation starts here
    // assume WellFormed($t0) at ./sources/auction.move:180:5+1
    assume {:print "$at(255,8603,8604)"} true;
    assume $IsValid'$2_tx_context_TxContext'($Dereference($t0));

    // assume WellFormed($t1) at ./sources/auction.move:180:5+1
    assume $IsValid'$2_tx_context_TxContext'($Dereference($t1));

    // trace_local[ctx1]($t0) at ./sources/auction.move:180:5+1
    $temp_0'$2_tx_context_TxContext' := $Dereference($t0);
    assume {:print "$track_local(105,17,0,$2_tx_context_TxContext):", $temp_0'$2_tx_context_TxContext'} $temp_0'$2_tx_context_TxContext' == $temp_0'$2_tx_context_TxContext';

    // trace_local[ctx2]($t1) at ./sources/auction.move:180:5+1
    $temp_0'$2_tx_context_TxContext' := $Dereference($t1);
    assume {:print "$track_local(105,17,1,$2_tx_context_TxContext):", $temp_0'$2_tx_context_TxContext'} $temp_0'$2_tx_context_TxContext' == $temp_0'$2_tx_context_TxContext';

    // $t2 := read_ref($t0) at ./sources/auction.move:181:18+4
    assume {:print "$at(255,8696,8700)"} true;
    $t2 := $Dereference($t0);

    // $t3 := tx_context::sender($t2) at ./sources/auction.move:181:18+13
    $t3 := $2_tx_context_sender$pure($t2);
    assume $IsValid'address'($t3);

    // $t4 := read_ref($t1) at ./sources/auction.move:181:35+4
    assume {:print "$at(255,8713,8717)"} true;
    $t4 := $Dereference($t1);

    // $t5 := tx_context::sender($t4) at ./sources/auction.move:181:35+13
    $t5 := $2_tx_context_sender$pure($t4);
    assume $IsValid'address'($t5);

    // $t6 := !=($t3, $t5) at ./sources/auction.move:181:32+2
    assume {:print "$at(255,8710,8712)"} true;
    $t6 := !$IsEqual'address'($t3, $t5);

    // prover::requires($t6)no_abort check at ./sources/auction.move:181:9+40
    call $0_prover_requires($t6);
    assert {:msg "assert_failed(255,8687,8727): code should not abort"} !$abort_flag;

    // $t7 := read_ref($t0) at ./sources/auction.move:182:17+4
    assume {:print "$at(255,8745,8749)"} true;
    $t7 := $Dereference($t0);

    // $t8 := tx_context::sender($t7) at ./sources/auction.move:182:17+13
    $t8 := $2_tx_context_sender$pure($t7);
    assume $IsValid'address'($t8);

    // $t9 := read_ref($t1) at ./sources/auction.move:182:34+4
    assume {:print "$at(255,8762,8766)"} true;
    $t9 := $Dereference($t1);

    // $t10 := tx_context::sender($t9) at ./sources/auction.move:182:34+13
    $t10 := $2_tx_context_sender$pure($t9);
    assume $IsValid'address'($t10);

    // $t11 := !=($t8, $t10) at ./sources/auction.move:182:31+2
    assume {:print "$at(255,8759,8761)"} true;
    $t11 := !$IsEqual'address'($t8, $t10);

    // prover::ensures($t11)no_abort check at ./sources/auction.move:182:9+39
    assert {:msg "assert_failed(255,8737,8776): prover::ensures does not hold"} $t11;
    assert {:msg "assert_failed(255,8737,8776): code should not abort"} !$abort_flag;

    // trace_local[ctx1]($t0) at ./sources/auction.move:182:48+1
    assume {:print "$at(255,8776,8777)"} true;
    $temp_0'$2_tx_context_TxContext' := $Dereference($t0);
    assume {:print "$track_local(105,17,0,$2_tx_context_TxContext):", $temp_0'$2_tx_context_TxContext'} $temp_0'$2_tx_context_TxContext' == $temp_0'$2_tx_context_TxContext';

    // trace_local[ctx2]($t1) at ./sources/auction.move:182:48+1
    $temp_0'$2_tx_context_TxContext' := $Dereference($t1);
    assume {:print "$track_local(105,17,1,$2_tx_context_TxContext):", $temp_0'$2_tx_context_TxContext'} $temp_0'$2_tx_context_TxContext' == $temp_0'$2_tx_context_TxContext';

    // return () at ./sources/auction.move:182:48+1
    $ret0 := $t0;
    $ret1 := $t1;
    return;
